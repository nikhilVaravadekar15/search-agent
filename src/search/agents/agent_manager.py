...
from src.search.agents.tools import internet_search
...
class AgentManager:
    ...
    async def stream_events(
        ...
        Args:
            thread_id (str): reference to conversation-thread model
            message_id (str): message to save conversation-thread details against
        ...
        try:
            search_logger.info(
                f"starting search for thread_id={thread_id}, message_id={message_id}"
            )
            ...
            search_logger.info(
                f"Created {agent.name} for thread_id={thread_id}, message_id={message_id} & Query={query}"
            )
                ...
                async for chunk in agent.astream(
                    ...
                    if chunk_type == "messages":
                        ...
                        if isinstance(message_chunk, AIMessageChunk):
                            ...
                            search_logger.info(
                                f"{msg} for thread_id={thread_id}, message_id={message_id}"
                            )
                            yield search_types.SseEvent(
                                mode=search_types.SseMode.THINKING,
                                message=msg,
            ...
            search_logger.error(
                f"HTTPException in message streaming for thread_id={thread_id}, message_id={message_id} & {str(e)}"
            )
            raise e
            ...
            search_logger.error(
                f"Client disconnected during stream, thread_id={thread_id}, message_id={message_id} & {str(ce)}"
            )
            yield search_types.SseEvent(
                mode=search_types.SseMode.ERROR,
            ...
            search_logger.error(
                f"Unhandled exception in message streaming for thread_id={thread_id}, message_id={message_id} & {str(e)}",
                exc_info=True,
            )
    ...
    async def run(
        ...
        Args:
            ...
            message_id (UUID): Unique identifier for this specific agent execution
        ...
        async def agent_worker():
            ...
            try:
                ...
                search_logger.info(
                    f"Agent completed successfully for thread_id={thread_id}, message_id={message_id}"
                )

            except asyncio.CancelledError:
                search_logger.info(
                    f"Agent cancelled for thread_id={thread_id}, message_id={message_id}, performing cleanup"
                )
                ...
                search_logger.error(
                    f"Agent error for thread_id={thread_id} message_id={message_id}: {e}",
                    exc_info=True,
                )
                await chunk_queue.put((search_types.SseMessageType.ERROR, str(e)))
                raise
            finally:
                self.running_agent_tasks.pop(message_id_str, None)
        ...
        try:
            ...
            self.running_agent_tasks[message_id_str] = agent_task
            ...
            while streaming:
                ...
                if await request.is_disconnected():
                    search_logger.info(
                        f"Client disconnected for thread_id={thread_id}, message_id={message_id}, cancelling agent"
                    )
                    ...
                    if agent_task and not agent_task.done():
                        ...
                        try:
                            ...
                            search_logger.info(
                                f"Agent task cancelled for message_id={message_id}"
                            )
                    ...
                    if chunk_type == search_types.SseMessageType.CHUNK:
                        ...
                        if chunk_content.mode == search_types.SseMode.METADATA:
                            try:
                                ...
                                sources.extend([])
                        elif chunk_type == search_types.SseMode.ERROR:
                        ...
                        search_logger.info(
                            f"Agent stream ended for thread_id={thread_id}, message_id={message_id}"
                        )
                        streaming = False
                        break
                    elif (
            ...
            await crud.add_message_to_thread(
                thread_id=thread_id,
            ...
            search_logger.error(
                f"Error in stream consumer for thread_id={thread_id}, message_id={message_id}: {str(e)}"
            )
            if agent_task and not agent_task.done():
                agent_task.cancel()
                try:
            ...
            del chunk_queue
            search_logger.debug(
                f"Cleaned up all resources for thread_id={thread_id} message_id={message_id}"
            )

    async def stop_stream_message(self, message_id: UUID) -> bool:
        """
        ...
        try:
            message_id_str = str(message_id)
            if message_id_str not in self.running_agent_tasks:
                return False

            task = self.running_agent_tasks.get(message_id_str)
            if task is None or task.done():
                return False

            # cancel and delete the message_id entry
            task.cancel()
            self.running_agent_tasks.pop(message_id_str, None)
            try:
                await task
